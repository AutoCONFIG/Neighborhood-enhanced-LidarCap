...
# from kaolin.models.PointNet2 import separate_xyz_and_features, PointNet2GroupingLayer, \
#     furthest_point_sampling, fps_gather_by_index, three_nn, three_interpolate
    ...
    def __init__(self,
        ...
        final_layer_modules = [
            module for module in [
                ...
                nn.BatchNorm1d(final_dim) if batchnorm else nn.GroupNorm(NUM_GROUPS,
                                                                         final_dim),
                nn.ReLU(),
                # nn.Dropout(0.5),
...
class PointNet2SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        assert (len(radii_list) == len(num_samples_list) == len(
            pointnet_layer_dims_list)), (
            'Dimension of radii_list ({}), num_samples_list ({}), pointnet_layer_dims_list ({}) must match'
                .format(len(radii_list), len(num_samples_list),
                        len(pointnet_layer_dims_list)))
    ...
    def forward(self, xyz, features=None):
        ...
        if self.num_points_out is not None:
            ...
            new_xyz = fps_gather_by_index(
                xyz.transpose(1, 2).contiguous(), new_xyz_idx.unsqueeze(1))
            new_xyz = new_xyz.squeeze().transpose(1, 2).contiguous()
