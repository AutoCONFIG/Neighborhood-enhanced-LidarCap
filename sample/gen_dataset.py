...
import sys
sys.path.append('/SAMSUMG4T/mqh/lidarcapv2')
...
ROOT_PATH = '/SAMSUMG8T/ljl/data/lidarcap/'
...
def foo(id, npoints):
    ...
    bg_filenames = [
        os.path.join(f'/SAMSUMG8T/ljl/data/lidarcap/pointclouds/{id}', os.path.basename(e).replace('.ply', '.pcd')) for
        e in segment_filenames]

    bgs = multiprocess.multi_func(
        read_point_cloud, MAX_PROCESS_COUNT, len(bg_filenames),
        'Load bg files', True, bg_filenames)

    whole_bg = []
    whole_bg_points_nums = []
    for pcd, bg in tqdm(list(zip(cur_point_clouds, bgs)), desc='Gen Background', ncols=60):
        mask = ((bg - pcd.mean(axis=0, keepdims=True)) ** 2).sum(axis=1) < 2 ** 2
        bg = bg[mask]
        bg, counts = np.unique(np.vstack([bg, pcd]), axis=0, return_counts=True)
        bg = bg[counts == 1]
        whole_bg_points_nums.append(len(bg))
        whole_bg.append(fix_points_num(bg, npoints))
    ...
    for i in tqdm(range(len(cur_poses))):
        ...
        v = smpl(torch.from_numpy(np_poses[np.newaxis, :]).cuda(), torch.from_numpy(np_betas[np.newaxis, :]).cuda())
    ...
    poses, betas, trans, point_clouds, points_nums, depths, full_joints, lidar_to_mocap_RT, rotmats, body_mask = \
        np.stack(poses), np.stack(betas), np.stack(trans), np.stack(point_clouds), np.stack(points_nums), depths, np.stack(full_joints), lidar_to_mocap_RT,  np.stack(rotmats), body_mask


    return poses, betas, trans, point_clouds, points_nums, depths, full_joints, lidar_to_mocap_RT, rotmats, body_mask, np.stack(whole_bg)
...
def dump(ids, npoints, name):
    ...
    for id in ids:
        ...
        poses, betas, trans, point_clouds, points_nums, depths, full_joints, lidar_to_mocap_RT, rotmats, body_mask, bg = foo(id, npoints)
        """
        import OVis
        ...
        whole_point_clouds = np.concatenate((whole_point_clouds, np.stack(point_clouds)))
        whole_bg = np.concatenate((whole_bg, bg))
    ...
    with h5py.File(os.path.join(extras_path, whole_filename), 'w') as f:
        ...
        f.create_dataset('backgrounds', data=whole_bg)
...
fix_to_seq16('/SAMSUMG8T/mqh/lidarcapv2/dataset/lidarcap_train_bg.hdf5', transet)
...
if __name__ == '__main__' and False:
    ...
    dump(args.ids, args.npoints, 'lidarcap_test_bg')
