...
from loss.nn_loss import get_nn
...
from sample.outlier_sample import in_convex_polyhedron
...
class Lidarcapv2_Dataset(Dataset):
            ...
            if index > dataset_max_index:
                ...
                if self.lidar_to_mocap_RT_flag:
                    if 'lidar_to_mocap_RT' in data:
                        ...
                        assert self.cfg.use_rot is False
                ...
                assert pose.shape == (seqlen, 72) and full_joints.shape == (seqlen, 24, 3) and human_points.shape == (seqlen, 512, 3), 'shape 不正确！'
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.concat_info:
            concat = np.concatenate((item['human_points'], item['back_pc'],item['human_boundary']), axis=1)
